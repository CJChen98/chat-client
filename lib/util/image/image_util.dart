...
Future<Uint8List> compressImageByNative(
    Uint8List bytes, PickedFile pickedFile) async {
  var quality = _imageQuality(File(pickedFile.path).lengthSync());

  return await FlutterImageCompress.compressWithList(bytes, quality: quality);
}

compressImageByDart(Uint8List bytes) {
  var image = decodeImage(bytes);
  var quality = _imageQuality(image.length);
  var result = encodeJpg(image, quality: quality) as Uint8List;
  return result;
}

const MB = 1024 * 1024;

_imageQuality(int size) {
  print(size);
